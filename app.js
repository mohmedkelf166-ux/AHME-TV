...
// 🔧 رابط الـ Proxy الآمن (يخفي بيانات السيرفر تماماً عن المتصفح)
...
// ==================== Data fetching ====================
// لم نعد نستقبل بيانات الحساب من Firebase — الـ Worker يخزنها بأمان ويرسل فقط النتائج
...
function loadAll(){
  ...
  fetchJSON('get_live_streams')
    .then(data => { allChannels = dedupe(Array.isArray(data) ? data : []); finish(); });

  fetchJSON('get_vod_streams')
    .then(data => { allMovies = dedupe(Array.isArray(data) ? data : []); finish(); });

  fetchJSON('get_series')
    .then(data => { allSeries = dedupe(Array.isArray(data) ? data : []); finish(); });

  // categories (non-blocking)
  fetchJSON('get_live_categories').then(data => { liveCats = mergeCats(data); buildPills('liveFilters', liveCats, 'live'); });
  fetchJSON('get_vod_categories').then(data => { movieCats = mergeCats(data); buildPills('moviesFilters', movieCats, 'movie'); });
  fetchJSON('get_series_categories').then(data => { seriesCats = mergeCats(data); buildPills('seriesFilters', seriesCats, 'series'); });

  setTimeout(() => { if (done < 3){ done = 3; finishLoading(); } }, 8000);
}

function mergeCats(data){
  const m = {};
  if (Array.isArray(data)) data.forEach(c => { if (c.category_id) m[c.category_id] = c.category_name; });
  return m;
}
...
function dedupe(arr){
  ...
  arr.forEach(item => {
    ...
    const key = id ? id + '_' + item.__acc : Math.random();
...
function posterCardHTML(item, type){
  ...
  const rating = item.rating ? `<div class="poster-rating"><i class="fa-solid fa-star"></i> ${item.rating}</div>` : '';
  return `<div class="poster-card" onclick="openSeriesDetail(${item.series_id},'${escapeAttr(name)}',${item.__acc})">
    <div class="poster-frame">
      ${rating}
      <button class="poster-fav ${isFav?'active':''}" onclick="event.stopPropagation();toggleFavSeries(${item.series_id},'${escapeAttr(name)}','${escapeAttr(img)}',${item.__acc})"><i class="${favIcon}"></i></button>
...
function itemUrl(item, type){
  // بدل الرابط الحقيقي (اللي فيه host/user/pass) بنبني رابط عبر الـ Worker
  // فيه بس رقم الحساب (__acc) ورقم المحتوى — بيانات السيرفر لا تظهر أبداً هنا
  if (type === 'live') return `${PROXY_URL}/?type=live&acc=${item.__acc}&id=${item.stream_id}`;
  if (type === 'movie') return `${PROXY_URL}/?type=movie&acc=${item.__acc}&id=${item.stream_id}&ext=${item.container_extension||'mp4'}`;
...
// ==================== Series detail ====================
window.openSeriesDetail = function(seriesId, name, accIndex){
  ...
  fetchJSON('get_series_info', `&series_id=${seriesId}&acc=${accIndex}`)
    .then(data => {
      ...
      Object.keys(data.episodes).forEach(seasonNum => {
        ...
        eps.forEach(ep => {
          ...
          const epUrl = `${PROXY_URL}/?type=series&acc=${accIndex}&id=${ep.id}&ext=${ep.container_extension||'mp4'}`;
...
window.toggleFavSeries = function(seriesId, title, img, accIndex){
  const url = String(seriesId);
  const idx = favorites.findIndex(f => f.url === url);
  if (idx >= 0){ favorites.splice(idx,1); Toast('تم الإزالة من القائمة'); }
  else { favorites.push({url, title, img, type:'series', accIndex, seriesId}); Toast('تم الإضافة إلى القائمة'); }
  saveFavs();
  refreshCurrentView();
};
...
function renderFavorites(){
  ...
  grid.innerHTML = favorites.map(f => {
    const isFav = true;
    if (f.type === 'series'){
      return `<div class="poster-card" onclick="openSeriesDetail(${f.seriesId},'${escapeAttr(f.title)}',${f.accIndex})">
        <div class="poster-frame">
          <button class="poster-fav active" onclick="event.stopPropagation();toggleFavSeries(${f.seriesId},'${escapeAttr(f.title)}','${escapeAttr(f.img)}',${f.accIndex})"><i class="fa-solid fa-heart"></i></button>
